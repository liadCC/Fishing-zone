...
namespace FishingZone.Boat
{
    ...
    ///
    /// While occupied the station captures the player's interaction focus, so looking at anything
    /// else on deck cannot steal the key that gets them out of the seat. The Player action map is
    /// deliberately left enabled throughout, because Interact lives on it.
    /// </summary>
    public class NavigatorStation : MonoBehaviour, IInteractable
    {
        ...
        {
            // Never strand a player parented to a station that is going away.
            if (IsOccupied)
            {
                Release();
            }
        ...
        public bool CanInteract(GameObject interactor)
        {
            ...
            // A second player cannot take a wheel someone else is holding, but the occupant must
            // always be able to interact in order to leave.
